...
---> 
<cfcomponent displaname="LocationCriterionService" hint="Service that returns LocationCriterion constants that match a specified name or list of criterion id(s) as specified in the input selector. Please note that filtering by date range is not supported." extends="utils">

<!--- create empty structure instance to store property data ---> 
<cfset variables["instance"]=StructNew()>


<cffunction name="get" access="public" returntype="any" hint="Returns a list of LocationCriterion's that match the specified selector. @param selector filters the LocationCriterion to be returned. @return A list of location criterion. @throws ApiException when there is at least one error with the request." output="No">
<cfargument name="selector" required="no" type="Selector" hint="This field is required and should not be null."> 
...
<cfset var stTemp="">
<cfset var oLocationCriterionServiceGet="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oLocationCriterionServiceGet=createObject("component","locationcriterionserviceget")>
		<cfset oLocationCriterionServiceGet.setSelector(selector)>
		
		<cfset variables["instance"]["get"]=oLocationCriterionServiceGet>
		...
		<cfset stResponse=getService().callWebService("LocationCriterionService",this)>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				...
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","LocationCriterion")>
			...
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an LocationCriterion[] object and set data inside returning object --->
				<cfset oTemp=createObject("component","LocationCriterion")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
				<cfthrow message="The api returned an empty result." detail="You can check the SOAPRequest and SOAPResponse data through the variables SOAPRequest and SOAPResponse. e.g. oLocationCriterionService.getService().SOAPRequest">
			</cfif>
...
<cffunction name="getServiceUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['serviceUrl']" default="https://adwords.google.com/api/adwords/cm/v201109/LocationCriterionService">
...
<cffunction name="getSandboxUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['sandboxUrl']" default="https://adwords-sandbox.google.com/api/adwords/cm/v201109/LocationCriterionService">
