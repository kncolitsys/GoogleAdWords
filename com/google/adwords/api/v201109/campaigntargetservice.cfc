...
<cffunction name="mutate" access="public" returntype="CampaignTargetReturnValue" hint="Mutates (sets) targets for specified campaign identified in the campaign operations. Note: When a campaign is created, its targeting options are also created. To add or remove targets, call mutate with the SET operator to update the target lists accordingly. The ADD and REMOVE operators are not supported. @param operations list of operations associating targets with campaign ids. @return the updated campaign targets, not necessarily in the same order in which they came in. @throws ApiException if problems occurred while adding campaign targeting information." output="No">
...
<cffunction name="getServiceUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['serviceUrl']" default="https://adwords.google.com/api/adwords/cm/v201109/CampaignTargetService">
...
<cffunction name="getSandboxUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['sandboxUrl']" default="https://adwords-sandbox.google.com/api/adwords/cm/v201109/CampaignTargetService">
