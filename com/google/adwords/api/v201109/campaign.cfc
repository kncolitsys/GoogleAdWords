...
<cfproperty name="startDate" required="no" type="string" hint="Date the campaign begins. On add, defaults to the current day in the parent account's local timezone. The date's format should be YYYYMMDD. This field can be selected using the value 'StartDate'.This field can be filtered on." seq="5">
<cfproperty name="endDate" required="no" type="string" hint="Date the campaign ends. On add, defaults to 20371231, which means the campaign will run indefinitely. To set an existing campaign to run indefinitely, set this field to 20371230. The date's format should be YYYYMMDD. This field can be selected using the value 'EndDate'.This field can be filtered on." seq="6">
...
<cfproperty name="campaignStats" required="no" type="CampaignStats" hint="Statistics for this campaign, returned by get() only if requested in the selector. This field is read only and should not be set. If this field is sent to the API, it will be ignored." seq="10">
