...
<cffunction name="mutate" access="public" returntype="BulkMutateJob" hint="Adds or updates a bulk mutate job. Use the &laquo;ADD&raquo; operator to submit a new job, and the &laquo;SET&raquo; operator to add additional parts to an existing job. The &laquo;DELETE&raquo; operator is not supported. From v201008 and later, use the &laquo;REMOVE&raquo; operator to cancel a job. Only jobs that still have pending additions of request parts may be canceled. Note: In the current implementation, the check for duplicate job keys is only 'best effort', and may not prevent jobs with the same keys from being accepted if they are submitted around the same instant. @param operation The operation to perform. @throws ApiException if problems occurred while creating or updating jobs @return The added or updated bulk mutate job." output="No">
...
<cffunction name="getServiceUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['serviceUrl']" default="https://adwords.google.com/api/adwords/job/v201109/BulkMutateJobService">
...
<cffunction name="getSandboxUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['sandboxUrl']" default="https://adwords-sandbox.google.com/api/adwords/job/v201109/BulkMutateJobService">
