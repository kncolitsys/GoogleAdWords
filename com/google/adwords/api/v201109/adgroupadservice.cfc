...
---> 
<cfcomponent displaname="AdGroupAdService" hint="Use this service to manage ads. Available ad types are subclasses of the base Ad type and are shown in the AdGroupAd.ad documentation. Here are some of the commonly used ad types: Ad The primary ad type used on both the search and display networks. A text ad contains a headline, two lines of text, and a destination URL. Ad A standard image ad. Ad A text ad specific to mobile devices. In addition, a mobile ad can contain a click-to-call phone number. Ad (AdWords Display Ad Builder) A flexible ad type that supports video ads, image search ads, and more. For a list of available templates, see the Template Ad Formats appendix." extends="utils">

<!--- create empty structure instance to store property data ---> 
<cfset variables["instance"]=StructNew()>


<cffunction name="get" access="public" returntype="AdGroupAdPage" hint="Returns a list of AdGroupAds. @param serviceSelector the selector specifying the AdGroupAds to return. @return the page containing the AdGroupAds that meet the criteria specified by the selector. @throws ApiException when there is at least one error with the request." output="No">
...
<cfset var stTemp="">
<cfset var oAdGroupAdServiceGet="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oAdGroupAdServiceGet=createObject("component","adgroupadserviceget")>
		<cfset oAdGroupAdServiceGet.setServiceSelector(serviceSelector)>
		
		<cfset variables["instance"]["get"]=oAdGroupAdServiceGet>
		...
		<cfset stResponse=getService().callWebService("AdGroupAdService",this)>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				...
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","AdGroupAdPage")>
			...
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an AdGroupAdPage object and set data inside returning object --->
				<cfset oTemp=createObject("component","AdGroupAdPage")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
				<cfthrow message="The api returned an empty result." detail="You can check the SOAPRequest and SOAPResponse data through the variables SOAPRequest and SOAPResponse. e.g. oAdGroupAdService.getService().SOAPRequest">
			</cfif>
...
<cffunction name="mutate" access="public" returntype="AdGroupAdReturnValue" hint="Applies the list of mutate operations (ie. add, set, remove): Add - Creates a new ad. The adGroupId must reference an existing ad group. The child Ad must be sufficiently specified by constructing a concrete ad type (such as TextAd) and setting its fields accordingly. Set - Updates an ad group ad. Except for status, ad group ad fields are not mutable. Status updates are straightforward - the status of the ad group ad is updated as specified. If any other field has changed, it will be ignored. If you want to change any of the fields other than status, you must make a new ad and then delete the old one. Remove - Removes the link between the specified AdGroup and Ad. @param operations the operations to apply @return a list of AdGroupAds where each entry in the list is the result of applying the operation in the input list with the same index. For an add/set operation, the return AdGroupAd will be what is saved to the db. In the case, of the remove operation, the return AdGroupAd will simply be an AdGroupAd containing an Ad with the id set to the Ad being deleted from the AdGroup." output="No">
...
<cfset var stProperty="">
<cfset var stTemp="">
<cfset var oAdGroupAdServiceMutate="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oAdGroupAdServiceMutate=createObject("component","adgroupadservicemutate")>
		<cfset oAdGroupAdServiceMutate.setOperations(operations)>
		
		<cfset variables["instance"]["mutate"]=oAdGroupAdServiceMutate>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				<!--- return an array of objects --->
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","AdGroupAdReturnValue")>
					<cfset oTemp.setObjectData(aResponse[j])>
					<cfset ArrayAppend(aTemp,oTemp)>
				</cfloop>
				<cfset oReturnValue=aTemp>
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an AdGroupAdReturnValue object and set data inside returning object --->
				<cfset oTemp=createObject("component","AdGroupAdReturnValue")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
...
<cffunction name="getServiceUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['serviceUrl']" default="https://adwords.google.com/api/adwords/cm/v201109/AdGroupAdService">
...
<cffunction name="getSandboxUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['sandboxUrl']" default="https://adwords-sandbox.google.com/api/adwords/cm/v201109/AdGroupAdService">
