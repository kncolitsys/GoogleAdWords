...
---> 
<cfcomponent displaname="CustomerSyncService" hint="Use this service to retrieve the changed entities for a customer account or campaign based on the given date range." extends="utils">

<!--- create empty structure instance to store property data ---> 
<cfset variables["instance"]=StructNew()>


<cffunction name="get" access="public" returntype="CustomerChangeData" hint="Campaign->AdGroup hierarchy containing information about the objects changed at each level. All Campaigns that are requested in the selector will be returned, regardless of whether or not they have changed, but unchanged AdGroups will be ignored." output="No">
<cfargument name="selector" required="no" type="CustomerSyncSelector" hint="This field is required and should not be null."> 
...
<cfset var stTemp="">
<cfset var oCustomerSyncServiceGet="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oCustomerSyncServiceGet=createObject("component","customersyncserviceget")>
		<cfset oCustomerSyncServiceGet.setSelector(selector)>
		
		<cfset variables["instance"]["get"]=oCustomerSyncServiceGet>
		...
		<cfset stResponse=getService().callWebService("CustomerSyncService",this)>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				...
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","CustomerChangeData")>
			...
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an CustomerChangeData object and set data inside returning object --->
				<cfset oTemp=createObject("component","CustomerChangeData")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
				<cfthrow message="The api returned an empty result." detail="You can check the SOAPRequest and SOAPResponse data through the variables SOAPRequest and SOAPResponse. e.g. oCustomerSyncService.getService().SOAPRequest">
			</cfif>
...
<cffunction name="getServiceUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['serviceUrl']" default="https://adwords.google.com/api/adwords/ch/v201109/CustomerSyncService">
...
<cffunction name="getSandboxUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['sandboxUrl']" default="https://adwords-sandbox.google.com/api/adwords/ch/v201109/CustomerSyncService">
