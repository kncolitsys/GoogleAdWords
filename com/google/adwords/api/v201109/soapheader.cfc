...
<cfproperty name="developerToken" required="no" type="string" hint="Developer token to identify that the person making the call has enough quota." seq="3">
<cfproperty name="userAgent" required="no" type="string" hint="UserAgent is used to track distribution of API client programs and application usage. The client is responsible for putting in a meaningful value for tracking purposes. To be clear this is not the same as an HTTP user agent." seq="4">
<cfproperty name="validateOnly" required="no" type="boolean" hint="Used to validate the request without executing it." seq="5">
<cfproperty name="partialFailure" required="no" type="boolean" hint="If true, API will try to commit as many error free operations as possible and report the other operations' errors. Ignored for non-mutate calls." seq="6">
