...
<cfproperty name="adGroupId" required="no" type="long" hint="ID of the associated ad group. Text ads in this ad group will be candidates for parameterized text replacement. This field is required and should not be null." seq="1">
<cfproperty name="criterionId" required="no" type="long" hint="ID of the associated Keyword criterion. The keyword must be in the same ad group as this AdParam. Text ads triggered by this keyword will have their {paramN:default-value snippet replaced by the contents of  insertionText. This field is required and should not be null." seq="2">
<cfproperty name="insertionText" required="no" type="string" hint="Numeric value to insert into the ad text. The following restrictions apply: Can use , or . as a separator, with an optional . or , (respectively) for fractional values. For example, 1,000,000.00 and 2.000.000,10 are valid. Can be prepended or appended with a currency symbol. For example, $99.99 and 200&pound; are valid. Can be prepended or appended with a currency code. For example, 99.99USD and EUR200 are valid. Can use %. For example, 1.0% and 1,0% are valid. Can use + or -. For example, -10.99 and 25+ are valid. Can use / between two numbers. For example 4/1 and 0.95/0.45 are valid. This field is required and should not be null when it is contained within Operators : SET. The length of this string should be between 1 and 25, inclusive." seq="3">
<cfproperty name="paramIndex" required="no" type="int" hint="Defines which parameterized snippet of ad text to replace. For example, a value of 1 indicates a replacement for the {param1:default-value token. This field must be between 1.0 and 2.0, inclusive. This field is required and should not be null." seq="4">
