...
---> 
<cfcomponent displaname="AdExtensionOverrideService" hint="Use this service to override a campaign's existing extension for specific ads within that campaign. The following conditions apply to ad extension overrides: Only location extensions can be overridden. Before adding an override, the location extension must already have been added to the ad's campaign using a extension. Once you set the location for an ad extension override, it cannot be updated. If you delete the ad's override, any new overrides will inherit the location information from the deleted one. For general information about ad extensions, see the AdWords Help Center." extends="utils">

<!--- create empty structure instance to store property data ---> 
<cfset variables["instance"]=StructNew()>


<cffunction name="get" access="public" returntype="AdExtensionOverridePage" hint="Returns a list of AdExtensionOverrides based on an AdExtensionOverrideSelector. @param selector the selector specifying the query @return the page containing the AdExtensionOverride which meet the criteria specified by the selector" output="No">
<cfargument name="selector" required="no" type="AdExtensionOverrideSelector"> 
...
<cfset var stTemp="">
<cfset var oAdExtensionOverrideServiceGet="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oAdExtensionOverrideServiceGet=createObject("component","adextensionoverrideserviceget")>
		<cfset oAdExtensionOverrideServiceGet.setSelector(selector)>
		
		<cfset variables["instance"]["get"]=oAdExtensionOverrideServiceGet>
		...
		<cfset stResponse=getService().callWebService("AdExtensionOverrideService",this)>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				...
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","AdExtensionOverridePage")>
			...
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an AdExtensionOverridePage object and set data inside returning object --->
				<cfset oTemp=createObject("component","AdExtensionOverridePage")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
				<cfthrow message="The api returned an empty result." detail="You can check the SOAPRequest and SOAPResponse data through the variables SOAPRequest and SOAPResponse. e.g. oAdExtensionOverrideService.getService().SOAPRequest">
			</cfif>
...
<cfset var stProperty="">
<cfset var stTemp="">
<cfset var oAdExtensionOverrideServiceMutate="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oAdExtensionOverrideServiceMutate=createObject("component","adextensionoverrideservicemutate")>
		<cfset oAdExtensionOverrideServiceMutate.setOperations(operations)>
		
		<cfset variables["instance"]["mutate"]=oAdExtensionOverrideServiceMutate>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				<!--- return an array of objects --->
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","AdExtensionOverrideReturnValue")>
					<cfset oTemp.setObjectData(aResponse[j])>
					<cfset ArrayAppend(aTemp,oTemp)>
				</cfloop>
				<cfset oReturnValue=aTemp>
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an AdExtensionOverrideReturnValue object and set data inside returning object --->
				<cfset oTemp=createObject("component","AdExtensionOverrideReturnValue")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
...
<cffunction name="getServiceUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['serviceUrl']" default="https://adwords.google.com/api/adwords/cm/v201109/AdExtensionOverrideService">
...
<cffunction name="getSandboxUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['sandboxUrl']" default="https://adwords-sandbox.google.com/api/adwords/cm/v201109/AdExtensionOverrideService">
