...
---> 
<cfcomponent displaname="ReportDefinitionService" hint="Note: As of v201109 this service is read-only. New report definitions are no longer allowed. A report definition describes the type, the range, and report. Additionally, you can specify predicates to filter which rows are returned in a generated report. Please note that sorting and paging are not supported for report downloads. To find out which report fields are available based on report type, see the Report Types appendix or call getReportFields. For more information about retrieving reports, see Reporting Basics." extends="utils">

<!--- create empty structure instance to store property data ---> 
<cfset variables["instance"]=StructNew()>


<cffunction name="get" access="public" returntype="ReportDefinitionPage" hint="Returns the list of report definitions that meet the selector criteria. @param selector Determines which report definitions to return. If empty, all report definitions will be returned. @return A list of report definitions. @throws ApiException if problems occurred while fetching report definitions information." output="No">
<cfargument name="selector" required="no" type="ReportDefinitionSelector" hint="This field is required and should not be null."> 
...
<cfset var stTemp="">
<cfset var oReportDefinitionServiceGet="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oReportDefinitionServiceGet=createObject("component","reportdefinitionserviceget")>
		<cfset oReportDefinitionServiceGet.setSelector(selector)>
		
		<cfset variables["instance"]["get"]=oReportDefinitionServiceGet>
		...
		<cfset stResponse=getService().callWebService("ReportDefinitionService",this)>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				...
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","ReportDefinitionPage")>
			...
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an ReportDefinitionPage object and set data inside returning object --->
				<cfset oTemp=createObject("component","ReportDefinitionPage")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
				<cfthrow message="The api returned an empty result." detail="You can check the SOAPRequest and SOAPResponse data through the variables SOAPRequest and SOAPResponse. e.g. oReportDefinitionService.getService().SOAPRequest">
			</cfif>
...
<cffunction name="getReportFields" access="public" returntype="any" hint="Returns the available report fields for a given report type. @param reportType The type of report. @return The list of available report fields. Each ReportDefinitionField encapsulates the field name, the field data type, and the enum values (if the field's type is enum). @throws ApiException if a problem occurred while fetching the ReportDefinitionField information." output="No">
<cfargument name="reportType" required="no" type="string" hint="This field is required and should not be null."> 
<cfset var stResponse=StructNew()>
<cfset var aResponse="">
<cfset var j="">
<cfset var aTemp=arrayNew(1)>
...
<cfset var stProperty="">
<cfset var stTemp="">
<cfset var oReportDefinitionServiceGetReportFields="">

	<!--- use errorhandling --->
	<cftry>

		<cfset oReportDefinitionServiceGetReportFields=createObject("component","reportdefinitionservicegetreportfields")>
		<cfset oReportDefinitionServiceGetReportFields.setReportType(reportType)>
		
		<cfset variables["instance"]["getReportFields"]=oReportDefinitionServiceGetReportFields>
		
		<cfset aProperties=arrayNew(1)>
		<!--- create the attribute collection for this property llection --->
		<cfset stProperty=structNew()/>
		<!--- set attribute values. --->
		<cfset stProperty["name"]="getReportFields"/>
		<!--- set attribute seq. --->
		<cfset stProperty["seq"]=1/>
		<!--- add this attribute to the property attribute collection array. --->
		<cfset arrayAppend(aProperties,stProperty)/>
		...
		<cfif ArrayLen(aResponse) gt 0>
			...
			<cfif isArray(aResponse)>
				<!--- return an array of objects --->
				<cfloop from="1" to="#arrayLen(aResponse)#" index="j">
					<cfset oTemp=createObject("component","ReportDefinitionField")>
					<cfset oTemp.setObjectData(aResponse[j])>
					<cfset ArrayAppend(aTemp,oTemp)>
				</cfloop>
				<cfset oReturnValue=aTemp>
			<cfelseif isStruct(aResponse) and not structIsEmpty(aResponse)>
				<!--- create empty instance of an ReportDefinitionField[] object and set data inside returning object --->
				<cfset oTemp=createObject("component","ReportDefinitionField")>
				<cfset oTemp.setObjectData(aResponse)>
				<cfset oReturnValue=oTemp>
			<cfelse>
...
<cffunction name="getServiceUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['serviceUrl']" default="https://adwords.google.com/api/adwords/cm/v201109/ReportDefinitionService">
...
<cffunction name="getSandboxUrl" access="public" returntype="any" output="No">
	...
	<cfparam name="variables['instance']['sandboxUrl']" default="https://adwords-sandbox.google.com/api/adwords/cm/v201109/ReportDefinitionService">
